...
| `measure_eag` *(v0.11.0-alpha 拡張)* | v0.7 mock path 保持 + `replay_source` optional param 追加 = SmellNet 互換 replay session を 与えると real dataset waveform を 返す (embedded fixture or external CSV)、 `data_source` field で mock / replay 区別、 `is_embedded_fixture` で fixture / external CSV 区別 |
| `load_smellnet_csv` / `load_embedded_fixture` / `get_replay_window` / `download_smellnet_stub` *(v0.11.0-alpha)* | Python API only (MCP tool 未 wire = v0.12+ candidate)。 SmellNet 互換 MOX time-series CSV load + window 抽出 (linear interp + wrap around)、 embedded 3 substance × 3 channel × 100 timestep synthetic fixture、 実 DL は stub (別 STEP directive 待ち) |
